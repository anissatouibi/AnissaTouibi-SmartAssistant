...
class BottomNavLayout extends StatefulWidget {
  ...
  })  : assert(pages.isNotEmpty, "At least 1 page is required"),
        assert(pageStack == null || pages.length > pageStack.peek() && pageStack.peek() >= 0,
...
class _BottomNavLayoutState extends State<BottomNavLayout> {
  ...
  void onPageSelected(int index) {
    ...
    // TODO change bail conditions for smart assistant
    // if (!context.read<UserCubit>().state.isAuthed! && index == 4) return;
  ...
  Future<bool> onWillPop() async {
    print('willpop from btmNavLayout');
    ...
    int previous = pageStack.peek();
    // Pop the top element from bottom navigation stack
    pageStack.pop();
    int current = pageStack.peek();

    // If the stack is not empty
    if (pageStack.stack.isNotEmpty) {
      // Set state to change the page
      setState(() {});
