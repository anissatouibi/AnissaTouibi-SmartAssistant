...
const Radius _kThumbRadius = Radius.zero;
...
const Radius _kSeparatorRadius = Radius.zero;
...
// vertex of the CustomCupertinoSlidingSegmentedControl's Rect.
//
...
const double _kCornerRadius = 0.0;
...
class _Segment<T> extends StatefulWidget {
  ...
  // Whether the thumb of the parent widget (CustomCupertinoSlidingSegmentedControl)
          ...
          child: DecoratedBox(
            decoration: BoxDecoration(
              color: _kSeparatorColor
                  .withOpacity(_kSeparatorColor.opacity * separatorOpacityController.value),
...
///
/// {@tool dartpad}
/// This example shows a [CustomCupertinoSlidingSegmentedControl] with an enum type.
///
...
class CustomCupertinoSlidingSegmentedControl<T> extends StatefulWidget {
  ...
  ///   @override
  ///   Widget build(BuildContext context) {
  ///     return CustomCupertinoSlidingSegmentedControl<int>(
  ///       children: children,
  ...
  @override
  State<CustomCupertinoSlidingSegmentedControl<T>> createState() => _SegmentedControlState<T>();
}

class _SegmentedControlState<T> extends State<CustomCupertinoSlidingSegmentedControl<T>>
    with TickerProviderStateMixin<CustomCupertinoSlidingSegmentedControl<T>> {
  late final AnimationController thumbController =
      AnimationController(duration: _kSpringAnimationDuration, value: 0, vsync: this);
  Animatable<Rect?>? thumbAnimatable;

  late final AnimationController thumbScaleController =
      AnimationController(duration: _kSpringAnimationDuration, value: 0, vsync: this);
  ...
  @override
  void didUpdateWidget(CustomCupertinoSlidingSegmentedControl<T> oldWidget) {
    super.didUpdateWidget(oldWidget);
...
class _RenderSegmentedControl<T> extends RenderBox
  ...
  Size _calculateChildSize(BoxConstraints constraints) {
    ...
    while (child != null) {
      childWidth = math.max(
          childWidth, child.getMaxIntrinsicWidth(double.infinity) + 2 * _kSegmentMinPadding);
      child = nonSeparatorChildAfter(child);
    }
    childWidth = math.min(
      childWidth,
  ...
  Size _computeOverallSizeFromChildSize(Size childSize, BoxConstraints constraints) {
    final int childCount = this.childCount ~/ 2 + 1;
    return constraints
        .constrain(Size(childSize.width * childCount + totalSeparatorWidth, childSize.height));
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (highlightedChildIndex != null) {
      ...
      final Rect newThumbRect =
          _kThumbInsets.inflateRect(childParentData.offset & selectedChild.size);
      ...
      _paintThumb(context, offset, thumbRect, borderRadius, shadowEnabled: shadowEnabled);
    } else {
  ...
  void _paintThumb(
      PaintingContext context, Offset offset, Rect thumbRect, BorderRadius? borderRadius,
      {required bool shadowEnabled}) {
    RRect thumbRRect;
    if (borderRadius != null) {
      thumbRRect = RRect.fromRectAndCorners(
      ...
      thumbRRect = RRect.fromRectAndRadius(thumbRect.shift(offset), _kThumbRadius);
    }
    if (shadowEnabled) {
