...
class RulerPicker extends StatefulWidget {
  ...
  const RulerPicker({
    ...
    this.scaleLineStyleList = const [
      ScaleLineStyle(scale: 0, color: Color.fromARGB(255, 188, 194, 203), width: 2, height: 32),
      ScaleLineStyle(color: Color.fromARGB(255, 188, 194, 203), width: 1, height: 20),
    ],
...
class RulerPickerState extends State<RulerPicker> {
  ...
  void initState() {
    ...
    scrollController =
        ScrollController(initialScrollOffset: initValueOffset > 0 ? initValueOffset : 0);
  ...
  Widget _buildRulerScaleLine(int index) {
    ...
    int scale = index % (widget.isFt ? 12 : 10);
  ...
  Widget _buildRulerScale(BuildContext context, int index) {
    return SizedBox(
      ...
      child: Stack(
        ...
        children: <Widget>[
          Align(alignment: Alignment.topCenter, child: _buildRulerScaleLine(index)),
          Positioned(
            bottom: 5,
            width: 100,
            left: -50 + _ruleScaleInterval / 2,
            child: index % (widget.isFt ? 12 : 10) == 0
                ? Container(
                    alignment: Alignment.center,
                    child: Text(
                      widget.onBuildRulerScaleText(index, getRulerScaleValue(index)),
  ...
  double getPositionByValue(num value) {
    ...
    for (RulerRange config in widget.ranges) {
      if (config.begin <= value && config.end >= value) {
        offsetValue += ((value - config.begin) / config.scale) * _ruleScaleInterval;
        break;
