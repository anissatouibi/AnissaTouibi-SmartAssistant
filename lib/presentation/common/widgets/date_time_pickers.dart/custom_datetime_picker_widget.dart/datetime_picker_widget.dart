...
import 'package:smart_assistant_app/presentation/common/widgets/date_time_pickers.dart/custom_datetime_picker_widget.dart/date_picker_widget.dart';
...
class _DateTimePickerWidgetState extends State<DateTimePickerWidget> {
  ...
  void initState() {
    super.initState();
    _selectedValue =
        widget.initDateTime!.day; // Initialize selected value when the widget initializes
  }
  ...
  Widget _renderDatePickerItemComponent(int value, String format) {
    bool isSelected = (value == _currDay && format == "dd") ||
        (value == _currMonth && format == "MMM") ||
        (value == _currYear) && format == "yyyy";
    // if (isSelected) {
    ...
    return Container(
      ...
      child: Text(
        DateTimeFormatter.formatDateTime(value, format, widget.locale).replaceAll(".", ""), //me
        style: isSelected
            ? AppTextStyles.px28wBold(context).toPrimaryRedColor
  ...
  void _changeDaySelection(int index) {
    ...
    if (_currDay != dayOfMonth) {
      _selectedValue = dayOfMonth; // Update the selected value
      _currDay = dayOfMonth;
      _changeTimeRange();
      _onSelectedChange();
    }
